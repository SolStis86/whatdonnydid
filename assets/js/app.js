const DATA = {
  scenarios: {
    lower: {
      label: "Lower bound",
      description: "A shorter disruption where inventories, alternative suppliers and policy support absorb part of the shock.",
      global: { ureaExposure: "12%", ammoniaTradeAtRisk: "9%", yieldPressure: "3–8%", foodPriceRisk: "Low–Medium", populationExposed: "420M" }
    },
    base: {
      label: "Base case",
      description: "A sustained 3–6 month squeeze where fertiliser and fuel prices stay elevated through key planting windows.",
      global: { ureaExposure: "24%", ammoniaTradeAtRisk: "18%", yieldPressure: "7–18%", foodPriceRisk: "Medium–High", populationExposed: "870M" }
    },
    upper: {
      label: "Upper bound",
      description: "A prolonged 6–12 month disruption with tighter shipping, weaker substitution and sharper affordability pressure.",
      global: { ureaExposure: "41%", ammoniaTradeAtRisk: "31%", yieldPressure: "14–34%", foodPriceRisk: "High", populationExposed: "1.6B" }
    }
  },
  regions: [
  ...
  timeline: [
    { month: "Jan", urea: 100, dap: 100, diesel: 100 },
    { month: "Feb", urea: 112, dap: 106, diesel: 118 },
    { month: "Mar", urea: 139, dap: 121, diesel: 141 },
    { month: "Apr", urea: 162, dap: 138, diesel: 155 },
    { month: "May", urea: 178, dap: 151, diesel: 164 },
    { month: "Jun", urea: 191, dap: 166, diesel: 171 }
  ],
  cropExposure: [
    { crop: "Maize", nitrogen: 44, phosphate: 18, potash: 13, energy: 25 },
    { crop: "Wheat", nitrogen: 39, phosphate: 22, potash: 10, energy: 29 },
    { crop: "Rice", nitrogen: 41, phosphate: 16, potash: 12, energy: 31 },
    { crop: "Canola", nitrogen: 46, phosphate: 20, potash: 12, energy: 22 },
    { crop: "Soy", nitrogen: 8, phosphate: 26, potash: 32, energy: 34 }
  ],
  cascade: [
    { label: "Hormuz disruption", detail: "Energy, shipping and Gulf fertiliser flows are constrained." },
    { label: "Gas / sulphur / ammonia shock", detail: "Core inputs for nitrogen and phosphate fertiliser tighten." },
    { label: "Fertiliser shortage + affordability", detail: "Supply and price stress reduce farmer access." },
    { label: "Reduced application", detail: "Farmers cut rates, delay purchases or switch crops." },
    { label: "Crop yield pressure", detail: "Losses emerge through planting and harvest cycles." },
    { label: "Food price pressure", detail: "Commodity, feed, meat, dairy and retail costs transmit." }
  ],
  sources: [
    { name: "IFPRI — Iran war impacts on fertiliser markets", url: "https://www.ifpri.org/blog/the-iran-wars-impacts-on-global-fertilizer-markets-and-food-production/" },
    { name: "IFPRI — Will the Iran crisis lead to food price spikes?", url: "https://www.ifpri.org/blog/will-the-iran-crisis-lead-to-another-round-of-food-price-spikes/" },
    { name: "EIA — Hormuz closure and production outages", url: "https://www.eia.gov/pressroom/releases/press586.php" },
    { name: "FAOSTAT — Fertilisers by nutrient", url: "https://www.fao.org/statistics/events/events-detail/fertilizers-by-nutrient.-july-2025-update/en" },
    { name: "NPKGRIDS — global crop fertiliser application rates", url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC11526156/" },
    { name: "MapSPAM — spatial crop production allocation model", url: "https://www.mapspam.info/" }
  ]
};
...
function setFirst(selector, text) { const node = document.querySelector(selector); if (node) node.textContent = text; }
...
function applyPublicCopy() {
  ...
  document.querySelectorAll(".section-heading p").forEach((node) => {
    node.textContent = node.textContent
      .replace("The model tracks", "This page tracks")
      .replace("The table ranks regions by a composite score using nutrient shortfall, yield-at-risk and food-stress values.", "The table ranks regions by nutrient shortfall, yield-at-risk and food-stress values.");
  });
...
function project(lon, lat, width, height) { return { x: ((lon + 180) / 360) * width, y: ((90 - lat) / 180) * height }; }
...
function renderMap() {
  ...
  for (let lon = -120; lon <= 120; lon += 60) {
    const p1 = project(lon, -72, width, height), p2 = project(lon, 72, width, height);
    graticule.push(`<line class="graticule" x1="${p1.x}" y1="${p1.y}" x2="${p2.x}" y2="${p2.y}"></line>`);
  }
  for (let lat = -60; lat <= 60; lat += 30) {
    const p1 = project(-175, lat, width, height), p2 = project(175, lat, width, height);
    graticule.push(`<line class="graticule" x1="${p1.x}" y1="${p1.y}" x2="${p2.x}" y2="${p2.y}"></line>`);
  }
  const land = `
    <ellipse class="land" cx="210" cy="205" rx="145" ry="105"></ellipse><ellipse class="land" cx="260" cy="342" rx="78" ry="112"></ellipse>
    <ellipse class="land" cx="505" cy="178" rx="68" ry="52"></ellipse><ellipse class="land" cx="520" cy="285" rx="88" ry="118"></ellipse>
    <ellipse class="land" cx="665" cy="210" rx="185" ry="118"></ellipse><ellipse class="land" cx="720" cy="330" rx="95" ry="92"></ellipse>
  ...
  const points = DATA.regions.map(region => {
    ...
    return `<g data-region="${region.id}" tabindex="0" role="button" aria-label="${region.name}, ${region.country}">
      ...
      <text class="risk-label" x="${(x + radius + 5).toFixed(1)}" y="${(y - radius - 8).toFixed(1)}">${region.name}</text>
    </g>`;
  }).join("");
  ...
  mount.querySelectorAll("[data-region]").forEach(node => {
    ...
    node.addEventListener("keydown", event => { if (event.key === "Enter" || event.key === " ") renderRegionDetail(region); });
  });
}
...
function renderRegionDetail(region) {
  ...
  const score = composite(region);
  mount.innerHTML = `<p class="eyebrow">Selected region</p><h3>${region.name}, ${region.country}</h3><div class="big-score" style="color:${riskColor(score)}">${score}</div>
    <p><strong>Composite risk score</strong> under the ${DATA.scenarios[activeScenario].label.toLowerCase()} scenario.</p>
    <div class="detail-grid"><div class="detail-row"><span>Nutrient shortfall</span><strong>${r.nutrientShortfall}%</strong></div><div class="detail-row"><span>Yield-at-risk</span><strong>${r.yieldAtRisk}%</strong></div><div class="detail-row"><span>Production risk</span><strong>${r.productionRiskMt} Mt</strong></div><div class="detail-row"><span>Food-stress score</span><strong>${r.foodStress} / 100</strong></div></div>
    <p><strong>Main crops:</strong> ${region.crops.join(", ")}</p><p>${region.driver}</p>`;
}

function renderTable() {
  const mount = document.getElementById("regions-table");
  if (!mount) return;
  mount.innerHTML = [...DATA.regions].sort((a, b) => composite(b) - composite(a)).map((region, index) => {
    const r = region[activeScenario];
    const score = composite(region);
    return `<tr><td><span class="score-pill" style="color:${riskColor(score)}">#${index + 1}</span></td><td><strong>${region.name}</strong><span>${region.country}</span></td><td>${region.crops.join(", ")}</td><td>${r.nutrientShortfall}%</td><td>${r.yieldAtRisk}%</td><td>${r.productionRiskMt} Mt</td><td><span class="score-pill" style="background:${riskColor(score)}22;color:${riskColor(score)}">${r.foodStress}</span></td></tr>`;
  }).join("");
}
...
function linePath(values, width, height, pad, key) {
  ...
  return values.map((d, i) => {
    ...
    const y = height - pad - (((d[key] - 90) / (max - 90)) * (height - pad * 2));
...
function renderCropChart() {
  ...
  if (!mount) return;
  const width = 600, height = 380, pad = 50, barHeight = 34, gap = 28;
  const colors = { nitrogen: "#ef4444", phosphate: "#f97316", potash: "#fbbf24", energy: "#38bdf8" };
  const bars = DATA.cropExposure.map((crop, i) => {
    let x = pad + 90;
    const y = 70 + i * (barHeight + gap);
    const segments = ["nitrogen", "phosphate", "potash", "energy"].map(key => { const w = crop[key] * 3.3; const rect = `<rect x="${x}" y="${y}" width="${w}" height="${barHeight}" rx="8" fill="${colors[key]}"></rect>`; x += w; return rect; }).join("");
    return `<text class="axis" x="${pad}" y="${y + 22}">${crop.crop}</text>${segments}`;
  }).join("");
